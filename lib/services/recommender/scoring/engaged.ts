...
export function getLastEngagedTime(graph: GraphService, userId: UserNodeId, contentId: ContentNodeId): number {
    const edge = graph.getEdge('engaged', userId, contentId);
    if (edge) {
        const now = Date.now();
